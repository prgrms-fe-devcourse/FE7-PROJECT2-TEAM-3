...
import DetailPostSkeleton from "../../components/ui/loading/DetailPostSkeleton.tsx";
import Card from "../../components/ui/Card.tsx";
import ImageModal from "../../components/ImageModal.tsx";
import DetailImageZoom from "../../components/DetailImageZoom.tsx";
...
export default function DetailPost() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // console.log("페치");
        setIsLoading(true);
        let userProfile: { _id: string; email: string } | null = null;
        // 사용자 정보 조회
        const {
          data: { user },
        ...
        // 사용자 profile이 null이 아닌 경우에만 비교
        if (userProfile) {
          setIsMyPost(post.user_id === userProfile._id);
        } else {
          setIsMyPost(false);
        }

        // 글쓴이 프로필 가져오기
        const { data: writerProfile, error: writerProfileError } =
          await supabase
            .from("profiles")
        ...
        if (writerProfileError) {
          console.error("글쓴이 프로필 불러오기 실패:", writerProfileError);
        } else {
          setNickname(writerProfile.display_name);
          setProfileImage(writerProfile.profile_image);
          setLevel(writerProfile.level);
        }
        ...
        if (imageError) {
          console.error("이미지 불러오기 실패:", imageError);
        } else if (imageRows) {
          // src 필드만 추출해서 상태에 넣기
        ...
        if (hashtagError) {
          console.error("해시태그 불러오기 실패:", hashtagError);
        } else {
        ...
        if (error) {
          ...
          if (
            commentsObj &&
            JSON.stringify(commentsObj) !== JSON.stringify(comments)
          ) {
  ...
  const handleAddComment = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 경험치 업데이트
      const profile = Array.isArray(commentData.profiles)
  ...
  if (isLoading) return <DetailPostSkeleton />;
  return (
    <>
            ...
            >
              <input
                ...
                className={twMerge(
                  ...
                  "bg-white text-[#1A1D25] placeholder-gray-400 text-sm",
      ...
      <ImageModal isOpen={isModalOpened} onClose={closeModal}>
        <DetailImageZoom src={modalImageSrc} />
      </ImageModal>
    </>
  );
}
